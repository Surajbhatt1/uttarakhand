...
// $itemName = $_SESSION['checkoutDetails']['PackageName']; 
// $itemPrice = $_SESSION['checkoutDetails']['PackagePrice']; 
// echo "<pre>";
// print_r($_SESSION);
// echo "</pre>";
// exit();
...
if($jsonObj->request_type == 'create_payment_intent'){ 
    ...
    $itemPrice = $jsonObj->itemPrice;

    $itemPriceCents = round($itemPrice*100); 
    $itemName = $jsonObj->itemName;
    ...
    if(!empty($payment_intent) && $payment_intent->status == 'succeeded'){ 
        ...
        if(!empty($row_id)){ 
            ...
            if($insert){ 
                ...
                // echo "aaaa$payment_id";
                // update payment id
                // if(isset($_SESSION['checkoutDetails'])){
                    // $bookingId = $_SESSION['checkoutDetails']['BookingId']; 
                    // $bookingId = $bookingId; 
                    $bookingId = $jsonObj->bookingId;

                    $updateQuery = "UPDATE `tblbooking` SET `payment_id` = ? WHERE `tblbooking`.`BookingId` = ?";

                    $stmt1 = $db->prepare($updateQuery); 
                    $stmt1->bind_param("dd", $payment_id,$bookingId); 
                    $insert1 = $stmt1->execute(); 
                // }
            } 
        } 
